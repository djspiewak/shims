...
trait EitherConverters {
  ...
  implicit def eitherAs[A, B]: AsScalaz[Either[A, B], A \/ B] with AsCats[A \/ B, Either[A, B]] =
    new AsScalaz[Either[A, B], A \/ B] with AsCats[A \/ B, Either[A, B]] {
...
trait FunctionKConverters {
  ...
  implicit def functionkAs[F[_], G[_]]: AsScalaz[FunctionK[F, G], F ~> G] with AsCats[F ~> G, FunctionK[F, G]] =
    new AsScalaz[FunctionK[F, G], F ~> G] with AsCats[F ~> G, FunctionK[F, G]] {
      def c2s(f: FunctionK[F, G]) = new (F ~> G) { def apply[a](fa: F[a]) = f(fa) }
      def s2c(f: F ~> G) = new FunctionK[F, G] { def apply[a](fa: F[a]) = f(fa) }
    }
}

trait FreeConverters extends MonadConversions {

  implicit def freeAs[S[_], A]: AsScalaz[cats.free.Free[S, A], scalaz.Free[S, A]] with AsCats[scalaz.Free[S, A], cats.free.Free[S, A]] =
    new AsScalaz[cats.free.Free[S, A], scalaz.Free[S, A]] with AsCats[scalaz.Free[S, A], cats.free.Free[S, A]] {

      def c2s(f: cats.free.Free[S, A]) =
        f.foldMap(new FunctionK[S, scalaz.Free[S, *]] { def apply[a](sa: S[a]) = scalaz.Free.liftF(sa) })(
          monadToCats(
            Capture(scalaz.Monad[scalaz.Free[S, *]]),
            OptionCapture(None)))

      def s2c(f: scalaz.Free[S, A]) =
        f.foldMap[cats.free.Free[S, *]](new (S ~> cats.free.Free[S, *]) { def apply[a](sa: S[a]) = cats.free.Free.liftF(sa) })(
...
trait IndexedStateTConverters extends MonadConversions {
  ...
  implicit def stateTAs[F[_]: cats.Monad, S1, S2, A]: AsScalaz[cats.data.IndexedStateT[F, S1, S2, A], scalaz.IndexedStateT[F, S1, S2, A]] with AsCats[scalaz.IndexedStateT[F, S1, S2, A], cats.data.IndexedStateT[F, S1, S2, A]] =
...
trait NELConverters {
  ...
  implicit def nelAs[A]: AsScalaz[cats.data.NonEmptyList[A], scalaz.NonEmptyList[A]] with AsCats[scalaz.NonEmptyList[A], cats.data.NonEmptyList[A]] =
    new AsScalaz[cats.data.NonEmptyList[A], scalaz.NonEmptyList[A]] with AsCats[scalaz.NonEmptyList[A], cats.data.NonEmptyList[A]] {
...
trait EitherKConverters {
  ...
  implicit def eitherKAs[F[_], G[_], A]: AsScalaz[cats.data.EitherK[F, G, A], scalaz.Coproduct[F, G, A]] with AsCats[scalaz.Coproduct[F, G, A], cats.data.EitherK[F, G, A]] =
    new AsScalaz[cats.data.EitherK[F, G, A], scalaz.Coproduct[F, G, A]] with AsCats[scalaz.Coproduct[F, G, A], cats.data.EitherK[F, G, A]] {
...
trait EitherTConverters extends EitherConverters {
  ...
  implicit def eitherTAs[F[_], A, B](implicit F: cats.Functor[F]): AsScalaz[cats.data.EitherT[F, A, B], scalaz.EitherT[F, A, B]] with AsCats[scalaz.EitherT[F, A, B], cats.data.EitherT[F, A, B]] =
    new AsScalaz[cats.data.EitherT[F, A, B], scalaz.EitherT[F, A, B]] with AsCats[scalaz.EitherT[F, A, B], cats.data.EitherT[F, A, B]] {
...
trait KleisliConverters {
  ...
  implicit def kleisliAs[F[_], A, B]: AsScalaz[cats.data.Kleisli[F, A, B], scalaz.Kleisli[F, A, B]] with AsCats[scalaz.Kleisli[F, A, B], cats.data.Kleisli[F, A, B]] =
    new AsScalaz[cats.data.Kleisli[F, A, B], scalaz.Kleisli[F, A, B]] with AsCats[scalaz.Kleisli[F, A, B], cats.data.Kleisli[F, A, B]] {
...
trait OptionTConverters {
  ...
  implicit def optionTAs[F[_], A]: AsScalaz[cats.data.OptionT[F, A], scalaz.OptionT[F, A]] with AsCats[scalaz.OptionT[F, A], cats.data.OptionT[F, A]] =
    new AsScalaz[cats.data.OptionT[F, A], scalaz.OptionT[F, A]] with AsCats[scalaz.OptionT[F, A], cats.data.OptionT[F, A]] {
...
trait ValidatedConverters {
  ...
  implicit def validatedAs[E, A]: AsScalaz[cats.data.Validated[E, A], scalaz.Validation[E, A]] with AsCats[scalaz.Validation[E, A], cats.data.Validated[E, A]] =
    new AsScalaz[cats.data.Validated[E, A], scalaz.Validation[E, A]] with AsCats[scalaz.Validation[E, A], cats.data.Validated[E, A]] {
...
trait ValidatedNELConverters extends ValidatedConverters with NELConverters {
  ...
  implicit def validatedNelAs[E, A]: AsScalaz[cats.data.ValidatedNel[E, A], scalaz.ValidationNel[E, A]] with AsCats[scalaz.ValidationNel[E, A], cats.data.ValidatedNel[E, A]] =
    new AsScalaz[cats.data.ValidatedNel[E, A], scalaz.ValidationNel[E, A]] with AsCats[scalaz.ValidationNel[E, A], cats.data.ValidatedNel[E, A]] {
...
trait OneAndConverters {
  ...
  implicit def oneAndAs[F[_], A]: AsScalaz[cats.data.OneAnd[F, A], scalaz.OneAnd[F, A]] with AsCats[scalaz.OneAnd[F, A], cats.data.OneAnd[F, A]] =
    new AsScalaz[cats.data.OneAnd[F, A], scalaz.OneAnd[F, A]] with AsCats[scalaz.OneAnd[F, A], cats.data.OneAnd[F, A]] {
...
trait MaybeConverters {
  ...
  implicit def maybeAs[A]: AsCats[scalaz.Maybe[A], Option[A]] =
    new AsCats[scalaz.Maybe[A], Option[A]] {
...
trait MaybeTConverters {
  ...
  implicit def maybeTAs[F[_], A](implicit F: scalaz.Functor[F]): AsCats[scalaz.MaybeT[F, A], cats.data.OptionT[F, A]] =
...
trait WriterTConverters {
  ...
  implicit def writerTAs[F[_], W, A]: AsScalaz[cats.data.WriterT[F, W, A], scalaz.WriterT[F, W, A]] with AsCats[scalaz.WriterT[F, W, A], cats.data.WriterT[F, W, A]] =
    new AsScalaz[cats.data.WriterT[F, W, A], scalaz.WriterT[F, W, A]] with AsCats[scalaz.WriterT[F, W, A], cats.data.WriterT[F, W, A]] {
...
trait IorConverters {
  ...
  implicit def iorAs[A, B]: AsScalaz[cats.data.Ior[A, B], A \&/ B] with AsCats[A \&/ B, cats.data.Ior[A, B]] =
    new AsScalaz[cats.data.Ior[A, B], A \&/ B] with AsCats[A \&/ B, cats.data.Ior[A, B]] {
...
trait ConstConverters {
  ...
  implicit def constAs[A, B]: AsScalaz[cats.data.Const[A, B], scalaz.Const[A, B]] with AsCats[scalaz.Const[A, B], cats.data.Const[A, B]] =
    new AsScalaz[cats.data.Const[A, B], scalaz.Const[A, B]] with AsCats[scalaz.Const[A, B], cats.data.Const[A, B]] {
...
trait CokleisliConverters {
  ...
  implicit def cokleisliAs[F[_], A, B]: AsScalaz[cats.data.Cokleisli[F, A, B], scalaz.Cokleisli[F, A, B]] with AsCats[scalaz.Cokleisli[F, A, B], cats.data.Cokleisli[F, A, B]] =
    new AsScalaz[cats.data.Cokleisli[F, A, B], scalaz.Cokleisli[F, A, B]] with AsCats[scalaz.Cokleisli[F, A, B], cats.data.Cokleisli[F, A, B]] {
...
trait RWSTConverters extends MonadConversions {
  ...
  implicit def rwstAs[F[_], E, L, S, A](implicit F: cats.Monad[F]): AsScalaz[cats.data.RWST[F, E, L, S, A], scalaz.RWST[F, E, L, S, A]] with AsCats[scalaz.RWST[F, E, L, S, A], cats.data.RWST[F, E, L, S, A]] =
